...
# Union merge loop
I want you to merge `master` or `main` into this branch, however there is a merge conflicts as we speak. If there's no merge conflict, then just merge in. You might need to pull `master`/`main` to get the newest changes before the merge conflict appear.
