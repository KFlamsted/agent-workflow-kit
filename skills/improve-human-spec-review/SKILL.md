...
3. Review one chunk at a time. For each:
   ...
   - restate it in ordinary language, keeping technical terms only when they are needed to preserve meaning;
   ...
   - use `Situation`, `Action`, `Expected result`, and `Exceptions` to summarize Gherkin scenarios when those labels make the behaviour easier to review;
...
4. In the same response, first present the complete chunk review from step 3, then ask the human to `Approve`, `Request changes`, `Discuss`, or `Skip`. Never ask for a chunk decision unless that response includes the chunk's source reference and plain-language restatement. Wait before continuing.
...
- For every chunk, use this order in one response: chunk identifier and source reference, plain-language restatement, any `Possible gap`, then the decision question.
- Make the decision question specific to the chunk just shown, for example: `What is your decision on Chunk 2: Error handling?`
- Never send a standalone decision question such as `What's your decision on Chunk 2?`, even if the chunk appeared in an earlier response.
...
- Keep one independently reviewable requirement, condition, or decision per list item.
- Prefer one short sentence per list item. Split conditions and consequences instead of combining them.
- Prefer a longer clear list over a shorter dense explanation.
...
- Prefer familiar words. Keep exact technical terms only when they affect the requirement or decision.
